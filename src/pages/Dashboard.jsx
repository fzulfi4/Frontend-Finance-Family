...
const Dashboard = () => {
  ...
  const activeTheme = CARD_THEMES[activeCardIndex % CARD_THEMES.length];

  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      <header className="flex justify-between items-center mb-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold mb-1">{t('welcomeBack')}, {user?.full_name?.split(' ')[0] || 'User'}!</h1>
          <p className="text-gray-400 text-sm md:text-base">{t('financialOverview')}</p>
        </div>
      </header>
        ...
        <div className="flex flex-col h-full justify-between relative z-10">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="btn border border-red-500/30 text-accent-red hover:bg-red-500/10 px-5 flex-1 sm:flex-none"
            >
              <ArrowDownRight size={18} /> {t('expense')}
            </button>
            <button
              onClick={() => setIsTransferModalOpen(true)}
              className="btn border border-blue-500/30 text-accent-blue hover:bg-blue-500/10 px-5 flex-1 sm:flex-none"
            >
      ...
      <div>
        ...
        {walletsLoading ? (
          ...
          /* Empty state: show add wallet card */
          <div className="flex justify-center">
            <button
              onClick={() => setIsWalletModalOpen(true)}
              className="
                w-full max-w-[380px] h-[200px] rounded-2xl
                border-2 border-dashed border-white/10 hover:border-white/25
                flex flex-col items-center justify-center gap-3
                text-gray-500 hover:text-gray-300
                transition-all duration-300 hover:bg-white/[0.02] cursor-pointer
              "
            >
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="relative w-full max-w-[460px]">
              ...
              {wallets.length > 1 && (
                <button
                  onClick={goToPrev}
                  className="absolute left-0 md:-left-14 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/5 hover:bg-white/15 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-all backdrop-blur-sm"
                >
                  <ChevronLeft size={20} />
                </button>
              )}
              ...
              <div className="relative h-[210px] flex items-center justify-center overflow-hidden">
                {wallets.map((wallet, idx) => {
                  ...
                  // Wrap-around logic for smooth feel
                  ...
                  const translateX = adjustedOffset * 110;
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`
                          relative w-[320px] md:w-[380px] h-[190px] rounded-2xl p-6
                      ...
                      >
                        ...
                        <div className="relative z-10 flex flex-col justify-between h-full">
                          <div className="flex justify-between items-start">
                            <CreditCard size={28} className="text-white/25" />
                            {isActive && (
                              <button
                          ...
                          <div>
                            ...
                            <div className="flex items-center justify-between">
                              <p className="text-sm text-white/70 font-medium truncate max-w-[200px]">{wallet.name}</p>
                              <span className="text-[10px] text-white/40 uppercase tracking-[0.2em] font-medium">{wallet.type}</span>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              {wallets.length > 1 && (
                <button
                  onClick={goToNext}
                  className="absolute right-0 md:-right-14 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/5 hover:bg-white/15 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-all backdrop-blur-sm"
                >
      ...
      <Card>
        ...
        {txLoading ? (
          ...
          <div className="text-center py-12">
            <ReceiptText size={40} className="mx-auto text-gray-600 mb-3" />
          ...
          <div className="space-y-1">
            {filteredTransactions.slice(0, 8).map(tx => {
              ...
              return (
                <div key={tx.id} className="flex justify-between items-center p-4 rounded-xl hover:bg-white/5 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-full ${isTransfer ? 'bg-blue-500/10 text-accent-blue' : isIncome ? 'bg-green-500/10 text-accent-green' : 'bg-red-500/10 text-accent-red'}`}>
                      {isTransfer ? <ArrowRightLeft size={20} /> : isIncome ? <ArrowUpRight size={20} /> : <ArrowDownRight size={20} />}
                    </div>
                    <div>
                      <p className="font-semibold flex items-center gap-2 group/title">
                      ...
                      <p className="text-xs text-gray-400">
                        {tx.transaction_date ? new Date(tx.transaction_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : t('unknownDate')}
                        {' • '}
                        {isTransfer
                  ...
                  <div className={`font-bold ${isTransfer ? 'text-accent-blue' : isIncome ? 'text-accent-green' : 'text-gray-100'}`}>
